...
public class RegisterNewUserImpl implements RegisterNewUser {
	...
	public RegisterNewUserImpl(UserRepository userRepository,
			RegisterNewUserRulesValidator registerNewUserRulesValidator) {
	...
	public void execute(UserDomain domain) {
		...
		var userEntity = UserEntity.create().setId(domain.getId())
				...
				.setEmail(domain.getEmail()).setUserType(UserTypeEntityMapper.INSTANCE.toEntity(domain.getUserType()))
				.setStatus(StatusEntityMapper.INSTANCE.toEntity(domain.getStatus())).setPassword(domain.getPassword());
